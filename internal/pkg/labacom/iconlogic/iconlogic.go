...
type LineInfo struct {
	...
	AwardIconIndex []int //返回中奖下标，243线消除使用
}
...
func (icon *Iconinfo) GetIcon(cheatvalue int64, lbcfg *config.LabaConfig, isfree bool, isSpecialStyle bool, oddFunc func(cheatvalue int64), iconFunc func(cheatvalue int64)) {
	...
	for i := 0; i < max_times; i++ {
		...
		//log.Tracef("iconFunc 之后：%+v",icon.Line)

		icon.Gettotalodds(lbcfg)
		//log.Tracef("Gettotalodds 之后：%+v",icon.Line)
		if oddFunc != nil {
			oddFunc(cheatvalue)
		}
	...
	///log.Tracef("GetIcon icon : %+v", icon)
}
...
func (icon *Iconinfo) Gettotalodds(lbcfg *config.LabaConfig) int {
	...
	if lbcfg.AwardType == 4 {
		...
		for i := 0; i < lbcfg.Line; i++ {
			...
			if iconid == int32(lbcfg.Wild.IconId) {
				wildcount = 1
			}
...
func (icon *Iconinfo) GetIconIter(lbcfg *config.LabaConfig, id int32, row int, arr []int, ex int, tw int) {
	tmpwild := tw
	wildcount := 0
	iconcount := make(map[int32]int)
	var AwardIncoIndex []int
	AwardIncoIndex = append(AwardIncoIndex, arr...)
	for i := 0; i < lbcfg.Line; i++ {
		iconid := id
		index := row*lbcfg.Line + i
	...
	if len(iconcount) == 0 {
		iconaward, _ := lbcfg.IconAward[int64(id)]
		if iconaward[int64(row-1)] != 0 {
			var li LineInfo
			li.Count = int32(row)
			li.Gold = iconaward[int64(row-1)]
			li.Index = id
			li.Start = 1
	...
	for key, v := range iconcount {
		extraodds := ex
		if v > 0 {
			extraodds = extraodds * v
		}
		iconid := key
		if int(iconid) == lbcfg.Wild.IconId {
			iconid = id
		}
		...
		if row+1 >= lbcfg.Row || v == 0 {
			...
			if iconaward[int64(row)] != 0 {
				var li LineInfo
				li.Count = int32(row + 1)
				li.Gold = iconaward[int64(row)]
				li.Index = iconid
				li.Start = 1
				li.WildCount = int32(tmpwild)
				li.AwardIconIndex = AwardIncoIndex
...
//全图分散图标个数
func (icon *Iconinfo) GetScatterIconCount(iconid int, lbcfg *config.LabaConfig) int {
	var count = 0
	for _, arr := range lbcfg.Matrix {
		for i := 0; i < len(arr); i++ {
			if icon.Iconarr[arr[i]] == int32(iconid) {
...
//九线拉王总倍数计算，左起连续压线，wild 在第一轴不算百搭，百搭不连可变奖励777图标10，免费游戏图标，奖金图标
func (icon *Iconinfo) GetOdds(lbcfg *config.LabaConfig) int {
	icon.Line = make([]LineInfo, 0)
	icon.Odds = 0
	for index, v := range lbcfg.LineBall {
		count := 0
