...
func (user *User) SendBetSuccessMessage(bet *baijiale.Bet) {
...
// 下注
func (user *User) Bet(bet *baijiale.Bet, TableBet [5]int64) bool {
	//判断下注下标和下注区域下标是否超出列表
	if bet.BetIndex < 0 || bet.BetType >= 5 || bet.BetType < 0 || bet.BetIndex >= int32(len(user.Rule.BetList)) {
	...
	if int64(user.Rule.BetMinLimit) > TotalAmount {
		...
		return false
	} else if TotalUserSingleSpaceAmount > user.Rule.SingleUserSingleSpaceLimit[bet.BetType%5] {
		//个人玩家单区域限红
		SendBetFailMessage("您已达到该区域的下注额度限制！", user)
		return false
	} else if TotalSingleSpaceAmount > user.Rule.AllUserSingleSpaceLimit[bet.BetType%5] {
		//所有玩家单区域限红
