...
func SendBetFailMessage(FailMessageDetail string, user *User) {
	SendBetFailMessage := new(BRTB.BetFailMessage)
	SendBetFailMessage.FailMessage = FailMessageDetail

	//log.Tracef("SendBetFailMessage %s", FailMessageDetail)

	err := user.User.SendMsg(int32(BRTB.SendToClientMessageType_BetFailID), SendBetFailMessage)
	if err != nil {
...
func (user *User) SendBetSuccessMessage(bet *BRTB.Bet) {
	SendSuccessMessage := new(BRTB.BetSuccessMessage)
...
// 下注
func (user *User) Bet(bet *BRTB.Bet, TableBet [25]int64) bool {
	//判断下注下标和下注区域下标是否超出列表
	if bet.BetIndex < 0 || bet.BetType >= 25 || bet.BetType < 0 || bet.BetIndex >= int32(len(user.Rule.BetList)) {
	...
	// 下注总金额
	TotalBetAmount := int64(user.Rule.BetList[bet.BetIndex])
	//g个人单区域下注总金额
	TotalUserSingleSpaceAmount := TotalBetAmount + user.BetArea[bet.BetType]
	//个人所有区域下注总金额
	TotalUserAllSpaceAmount := TotalBetAmount + user.TotalBet
	//单区域下注总金额
	TotalSingleSpaceAmount := TotalBetAmount + TableBet[bet.BetType]
	//所有区域下注总金额
	...
	if int64(config.BRTBConfig.Betmin) > TotalAmount {
		str := fmt.Sprintf("至少携带%d金币才能下注！", config.BRTBConfig.Betmin/100)
		...
		return false
	} else if TotalUserSingleSpaceAmount > user.Rule.SingleUserSingleSpaceLimit[bet.BetType] {
		//个人玩家单区域限红
		SendBetFailMessage("您已达到该区域的下注额度限制！", user)
		return false
	} else if TotalSingleSpaceAmount > user.Rule.AllUserSingleSpaceLimit[bet.BetType] {
		//所有玩家单区域限红
