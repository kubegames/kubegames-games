...
type User struct {
	...
	TotalBet       int64             // 总下注金额  局结束后重置数据
	BetInfo        [4]int64          //4个区域的下注信息  局结束后重置数据
	WinCount       int               //赢的统计
	RetCount       []bool            //结果统计
	BetCount       []int64           //下注统计
	RetWin         int               //20局赢统计
	RetBet         int64             //20局下注统计
	NoBetCount     int               //用户无下注统计（）
	Rule           *config.RoomRules //房间规则
	SettleMsg      *BRZJH.SettleMsg  //结算消息
...
func (user *User) SendBetSuccessMessage(bet *BRZJH.Bet) {
	SendSuccessMessage := new(BRZJH.BetSuccessMessage)
	...
	//user.User.SendMsg(int32(BRZJH.SendToClientMessageType_BetSuccessMessageID), SendSuccessMessage)
	user.Table.Broadcast(int32(BRZJH.SendToClientMessageType_BetSuccessMessageID), SendSuccessMessage)

}

// 下注
func (user *User) Bet(bet *BRZJH.Bet, TableBet [4]int64) bool {
	//判断下注下标和下注区域下标是否超出列表
	...
	if int64(user.Rule.BetMinLimit) > TotalAmount {
		...
		return false
	} else if (user.Rule.Zhuang != 0 && TotalBet+TotalBetAmount > user.Rule.Zhuang) || TotalBetAmount+user.TotalBet > (TotalAmount+user.TotalBet) {
		...
		//user.User.SendMsg(int32(BRZJH.SendToClientMessageType_BetRet),bet)
		user.NoBetCount = 0
		return true
	}
}
