...
func (si *SceneInfo) SitScene(user *User, SeatNum int) bool {
	...
	if ok1 {
		//发送坐下位置失败
		msg := new(rbwar.UserSitDownFail)
		msg.FailReaSon = "你坐下的位置已经被其他玩家捷足先登了！"

		user.User.SendMsg(int32(rbwar.SendToClientMessageType_SitDownFail), msg)
		return false
	}

	//if user.User.GetScore() < int64(si.Rule.SitDownLimit) {
	//	msg := new(rbwar.UserSitDownFail)
	//	str := fmt.Sprintf("低于%d不能入座！", si.Rule.SitDownLimit/100)
	//	msg.FailReaSon = str
	//
	//	user.User.SendMsg(int32(rbwar.SendToClientMessageType_SitDownFail), msg)
	//	return false
...
func (si *SceneInfo) UserStandUp(user player.PlayerInterface) {
	...
	if ok {
		log.Debugf("删除座位上的用户：%v", user.GetID())
