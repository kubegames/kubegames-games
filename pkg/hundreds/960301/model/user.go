...
type User struct {
	...
	SettleMsg      *rbwar.SettleMsg  //结算消息
...
func SendBetFailMessage(FailMessageDetail string, user *User) {
	SendBetFailMessage := new(rbwar.BetFailMessage)
	SendBetFailMessage.FailMessage = FailMessageDetail

	if !user.User.IsRobot() {
		log.Debugf("SendBetFailMessage %s", FailMessageDetail)
	}

	err := user.User.SendMsg(int32(rbwar.SendToClientMessageType_BetFailID), SendBetFailMessage)
	if err != nil {
		log.Debugf("SendBetFailMessage Error, %s, %s", FailMessageDetail, err.Error())
		return
	}
}

func (user *User) SendBetSuccessMessage(bet *rbwar.Bet) {
	SendSuccessMessage := new(rbwar.BetSuccessMessage)
	...
	user.Table.Broadcast(int32(rbwar.SendToClientMessageType_BetSuccessMessageID), SendSuccessMessage)
...
// 下注
func (user *User) Bet(bet *rbwar.Bet, TableBet [3]int64) bool {
	//判断下注下标和下注区域下标是否超出列表
	...
	case RED:
		tempSpceAmount = user.BetRed
	case BLACK:
		tempSpceAmount = user.BetBlack
	case ONEHIT:
		tempSpceAmount = user.BetLuck
	}
	...
	// 主要判断总金额与下注金额的关系
	// if !user.User.IsRobot() {
	// 	log.Debugf("用户余额为：%v", TotalAmount)
	// }
	...
	if int64(user.Rule.BetMinLimit) > TotalAmount {
		...
		if bet.BetType == 0 {
			user.BetRed += TotalBetAmount
		} else if bet.BetType == 1 {
			user.BetBlack += TotalBetAmount
		} else if bet.BetType == 2 {
			user.BetLuck += TotalBetAmount
		}

		// if !user.User.IsRobot() {
		// 	log.Debugf("玩家%v下注 红:%v, 黑：%v，幸运：%v", user.User.GetID(), user.BetRed, user.BetBlack, user.BetLuck)
		// }
