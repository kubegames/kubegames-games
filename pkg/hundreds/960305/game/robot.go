...
func (r *Robot) OnGameMessage(subCmd int32, buffer []byte) {
	switch subCmd {
	case int32(baijiale.SendToClientMessageType_Status):
		{
			r.LastBetPlace = -1
			r.OnStatusMsg(buffer)
		}
		break
	case int32(baijiale.SendToClientMessageType_SendRobotMiPai):
		r.SendRobotMiPai()
	}
}
...
func (r *Robot) OnStatusMsg(b []byte) {
	msg := &baijiale.StatusMessage{}
	proto.Unmarshal(b, msg)
	if msg.Status == int32(baijiale.GameStatus_BetStatus) {
		r.BetCount = 0
		r.AddBetTimer()
	} else if msg.Status == int32(baijiale.GameStatus_EndBetMovie) {
		if r.TimerJob != nil {
			// r.TimerJob.Cancel()
			r.TimerJob = nil
		}
...
func (r *Robot) ChoseBetRorB() int {
	if r.GameLogic.SenceSeat.IsSitDown(r.User) && r.LastBetPlace != -1 {
		return r.LastBetPlace
	}

	line := r.GameLogic.BetTotal[ZHUANG] - r.GameLogic.BetTotal[XIAN]
...
func (r *Robot) SendBetMsg(place int, index int) {
	r.BetCount++
	msg := new(baijiale.Bet)
	msg.BetIndex = int32(index)
	msg.BetType = int32(place)
	//	log.Tracef("发送机器人下注")
	r.User.SendMsgToServer(int32(baijiale.ReceiveMessageType_BetID), msg)
}
