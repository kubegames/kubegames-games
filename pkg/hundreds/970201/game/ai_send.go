...
func (game *Game) AiSendNew() {
	...
	go func() {
		...
		if aiSend == nil {
			fmt.Println("aiSend config 为空，红包数量：",redCount)
			return
		}
		redCountIndex := GetSendRedIndex(aiSend)
		//fmt.Println("redCountIndex : ",redCountIndex,aiSend.SendCount)
		...
		//fmt.Println("还剩 ",sendRedCount," 个红包没发完")
...
func GetSendRedIndex(aiSend *config.AiSendNew) int {
	...
	//fmt.Println("发送红包数量的概率随机值：",index)
	tmpSendCountRate := 0
