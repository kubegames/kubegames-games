...
func (r *Robot) OnGameMessage(subCmd int32, buffer []byte) {
	switch subCmd {
	case int32(BRNN.SendToClientMessageType_Status):
		{
			r.LastBetPlace = -1
...
func (r *Robot) OnStatusMsg(b []byte) {
	msg := &BRNN.StatusMessage{}
	proto.Unmarshal(b, msg)
	if msg.Status == int32(BRNN.GameStatus_BetStatus) {
		r.BetCount = 0
		r.AddBetTimer()
	} else if msg.Status == int32(BRNN.GameStatus_EndBetMovie) {
		if r.TimerJob != nil {
			r.TimerJob.Cancel()
			r.TimerJob = nil
		}
...
func (r *Robot) SendBetMsg(place int, index int) {
	...
	//	log.Tracef("发送机器人下注")
	r.User.SendMsgToServer(int32(BRNN.ReceiveMessageType_BetID), msg)
}
