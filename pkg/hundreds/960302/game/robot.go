...
func (r *Robot) OnGameMessage(subCmd int32, buffer []byte) {
	switch subCmd {
	case int32(longhu.SendToClientMessageType_Status):
		{
			r.LastBetPlace = -1
...
func (r *Robot) OnStatusMsg(b []byte) {
	msg := &longhu.StatusMessage{}
	proto.Unmarshal(b, msg)
	if msg.Status == int32(longhu.GameStatus_BetStatus) {
		r.BetCount = 0
		r.AddBetTimer()
...
func (r *Robot) ChoseBetRorB() int {
	...
	line := r.GameLogic.BetTotal[LONG] - r.GameLogic.BetTotal[HU]
...
func (r *Robot) SendBetMsg(place int, index int) {
	...
	//	log.Tracef("发送机器人下注")
	r.User.SendMsgToServer(int32(longhu.ReceiveMessageType_BetID), msg)
}
