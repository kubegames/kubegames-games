...
import (
	...
	"github.com/kubegames/kubegames-games/pkg/slots/970102/msg"
...
func (game *Game) CloseTable() {
	for _, user := range game.Users {
		if game.Status != msg.GameStatus_game_End {
			game.count(user.user)
		}
		delete(game.Users, user.user.GetID())
		...
		game.table.UploadPlayerRecord([]*platform.PlayerRecord{
			{
				...
				GameNum:      game.table.GetGameNum(),
				ProfitAmount: user.Score - int64(user.AllBet),
				BetsAmount:   int64(user.AllBet),
				DrawAmount:   int64(user.Gold) - user.Score,
				OutputAmount: user.Score,
...
func (game *Game) OnActionUserSitDown(user player.PlayerInterface, chairId int, cfg string) table.MatchKind {
	...
	game.table.UploadPlayerRecord([]*platform.PlayerRecord{
		{
			...
			GameNum:      game.table.GetGameNum(),
			ProfitAmount: u.Score - int64(u.AllBet),
			BetsAmount:   int64(u.AllBet),
			DrawAmount:   int64(u.Gold) - u.Score,
			OutputAmount: u.Score,
...
func (game *Game) count(user player.PlayerInterface) {
	...
	if u != nil {
		...
		if pokerType != msg.PokerType_ZERO {
			...
			gold = multiple * u.Bet
			score = user.SetScore(game.table.GetGameNum(), int64(gold), game.table.GetRoomRate())
		}
		u.Gold += gold
		u.Score += score
