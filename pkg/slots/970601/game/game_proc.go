...
func (game *Game) ProcStartGame(buffer []byte, user *data.User) {
	...
	user.User.SetScore(user.Table.GetGameNum(), -score, game.Table.GetRoomRate())
	user.User.SendMsg(int32(msg.S2CMsgType_START_GAME_RES), &msg.S2CUserScore{
...
func (game *Game) ProcChooseCaijin(buffer []byte, user *data.User) {
	...
	game.user.User.SetScore(game.Table.GetGameNum(), score, game.Table.GetRoomRate())
	user.User.SendMsg(int32(msg.S2CMsgType_RES_CAIJIN), &msg.S2CCaijin{
		Score: score,
	})
