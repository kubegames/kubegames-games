...
func (game *Game) EndGame() {
	...
	if game.EndGameInfo.WinScore > 0 {
		...
		_, _ = game.user.User.SetScore(game.Table.GetGameNum(), game.EndGameInfo.WinScore, game.Table.GetRoomRate())
		game.user.User.SendRecord(game.Table.GetGameNum(), game.EndGameInfo.WinScore-game.BottomCount*game.Bottom,
			game.BottomCount*game.Bottom, taxScore, game.EndGameInfo.WinScore, "") //1月15新加的据结束，用户离开调用
	} else {
		game.user.User.SendRecord(game.Table.GetGameNum(), -game.BottomCount*game.Bottom,
