...
import (
	"fmt"
	_777 "go-game-sdk/example/game_LaBa/990501/msg"
...
func (g *Game) CloseTable() {
	if g.user != nil {
		if g.FreeGameTimes != 0 {
			str := fmt.Sprintf("%v,%v,%v,%v", g.FreeGameTimes, g.FreeGameGold, g.LastWildIndex, g.LastBet)
...
func (g *Game) OnGameMessage(subCmd int32, buffer []byte, user player.PlayerInterface) {
	switch subCmd {
	case int32(_777.MsgIDC2S_Bet):
		g.OnUserBet(buffer)
		break
	case int32(_777.MsgIDC2S_AskSence):
		g.SendScene(user)
		break
	}
}
...
func (g *Game) SendScene(user player.PlayerInterface) bool {
	...
	user.SendMsg(int32(_777.ReMsgIDS2C_SenceID), senddata)
	g.table.StartGame()
