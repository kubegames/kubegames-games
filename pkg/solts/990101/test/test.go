...
func Test(lb *config.LabaConfig, xml *xiaomali.XiaoMaLiCfg) {
	var tres testres
	...
	for i := 0; i < total; i++ {
		...
		//这里算的图形
		for {
			freegame, smallgame := g.GetIconRes(int64(tres.tc.cheatvalue))
		...
		//freegame, _ := g.GetIconRes(int64(tres.tc.cheatvalue))
		//如果两次免费不一样就表示新的免费
...
func (tres *testres) littlegame(odds int) {
	...
	// arr := [...]int{0, 1 * lineNum, 3 * lineNum, 5 * lineNum, 10 * lineNum, 20 * lineNum, 50 * lineNum, 99999999}
