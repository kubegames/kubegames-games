...
import (
	"fmt"
	roomconfig "go-game-sdk/example/game_LaBa/990301/config"
	"go-game-sdk/example/game_LaBa/990301/gamelogic"
...
func Test(lb *config.LabaConfig) {
	...
	for i := 0; i < total; i++ {
		...
		if g.EnterFreeGame {
			g.FreeGameIndex = rand.Int31n(5)
			cheat := roomconfig.RoomCfg.GetFreeGameWeight(int32(tres.tc.cheatvalue))
			g.FreeGameTimes += cheat.Fgw[g.FreeGameIndex].FreeGameTimes
			total += g.FreeGameTimes
			tres.freegamefun(g.FreeGameTimes)
...
func (tres *testres) freegame(odds int) {
	...
	arr := [...]int{0, 1 * lineNum, 3 * lineNum, 5 * lineNum, 10 * lineNum, 30 * lineNum, 50 * lineNum, 100 * lineNum, 200 * lineNum, 999999999}
...
func (tres *testres) littlegame(odds int) {
	...
	arr := [...]int{0, 10 * lineNum, 30 * lineNum, 50 * lineNum, 100 * lineNum, 200 * lineNum, 500 * lineNum, 999999999}
...
func (tres *testres) normalgame(odds int) {
	...
	//arr := [...]int{0, 1, 3, 5, 10, 30, 50, 100, 199, 99999999}
	for i := 0; i < len(arr); i++ {
		if odds <= arr[i] {
			tres.odds[i]++
			break
...
func (tres *testres) writefile() {
	...
	file.WriteString(str)
	arr2 := [...]int{0, 10, 30, 50, 100, 200, 500, 99999999}
