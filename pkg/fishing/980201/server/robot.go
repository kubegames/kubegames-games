...
import (
	"go-game-sdk/example/game_buyu/980201/config"
...
func (robot *Robot) startShoot() {
	if !robot.CanShoot || robot.BulletNum >= 15 {
		log.Warnf("shoot worn", robot.CanShoot, robot.BulletNum, robot.AI.GetID())
		return
	}
	req := &msg.ShootReq{
		UserId: robot.AI.GetID(),
...
func (robot *Robot) checkModel() {
	...
	if rand < lockChance {
		lock = true
	} else if rand < lockChance+config.GetRobotPenetrationModelChance() {
		per = true
	}
...
func (robot *Robot) shoot(buffer []byte) {
	//res := &msg.ShootRes{}
	//proto.Unmarshal(buffer, res)
	//if !robot.IsPer && robot.AI.GetID() == res.GetUserId() {
	//	robot.BulletNum++
	//}
}
...
func (robot *Robot) changeShootStaut() {
	...
	robot.AI.AddTimer(int64(3000), func() {
		robot.CanShoot = true
	})
}
