...
func GetSceneBossFishCap(islandId, key string) map[msg.Type]int {
	...
	fishCap[msg.Type_SPECIAL] = int(GetFishScore(GetBossId(islandId, key))) * 2
	fishCap[msg.Type_SMALL] = all * small / 100
	fishCap[msg.Type_MIDDLE] = all * middle / 100
	fishCap[msg.Type_BIG] = all * big / 100
	return fishCap
...
func GetSkillId(fishId string) int32 {
	...
	if fishId == "24" || fishId == "25" || fishId == "26" || fishId == "27" || fishId == "28" {
		return 4
	}
	return 0
}
...
func GetConfLine(fishId string) ([]interface{}, []interface{}, interface{}, interface{}, int) {
	...
	line, speed, t, name, r := GetLine(lines)
	if key == "zhixian(2)" || key == "zhixian(1)" {
		r = -1
	}
...
func GetRobotPenetrationModelChance() int {
	num, err := RobotConf.Get("1").Get("1").Get("penetrationModelChance").Int()
	if err != nil {
		log.Errorf("robot penetrationModelChance err", err)
	}
	return num
}
