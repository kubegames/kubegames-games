...
func (table *TableLogic) OnActionUserSitDown(user player.PlayerInterface, chairId int, cfg string) tableInterface.MatchKind {
	if config.GetFishBet(cfg, 1) <= 0 {
		log.Warnf("config err %s", cfg)
		return tableInterface.SitDownErrorOver
	}
	if len(table.Users) == 0 && user.IsRobot() {
		log.Warnf("fisrst user is robot")
		return tableInterface.SitDownErrorOver
	}
	if table.bossTime && !user.IsRobot() {
		log.Warnf("fisrst user is not robot but bossTime %v", table.bossTime)
		return tableInterface.SitDownErrorNomal
	}
	if len(table.Users) > 4 {
		log.Warnf("user > 4")
		return tableInterface.SitDownErrorNomal
	}
	now := time.Now().UnixNano() / 1e6
	if now-table.LastDismissTime < 10000 {
		log.Warnf("now-table.LastDismissTime < 10000 %d", table.LastDismissTime)
		return tableInterface.SitDownErrorNomal
	}
	if now-table.userExist[user.GetID()] < 60000 {
		log.Warnf("now-table.userExist  < 60000 %v", table.userExist)
		return tableInterface.SitDownErrorNomal
	}
...
func (table *TableLogic) CloseTable() {
	...
	for _, u := range table.Users {
		...
		u.WriteLog()
		gameNum := u.GameNum
		u.InnerUser.SendRecord(gameNum, u.OutputAmount+u.Bet, -u.Bet, u.Win-u.OutputAmount, u.OutputAmount, "")
	}
...
func (table *TableLogic) stopTimer() {
	for key, v := range table.timer {
		if v != nil {
			table.Table.DeleteJob(v)
			delete(table.timer, key)
		}
	}
}

//进入房间
func (table *TableLogic) enterRoom(user player.PlayerInterface) {
	...
	user2.Log = make([]*message.GameLog, 0)
	if user.IsRobot() {
...
func (table *TableLogic) tax(coinChange int64) int64 {
	if coinChange > 0 {
		coin := coinChange * (10000 - table.RoomRate) / 10000
		return coin
	}
	return coinChange
}
...
func (table *TableLogic) updateScore(user *data.User, coinChange int64) {
	//og.Debugln("user update score :", coinChange)
	...
	//	betAmount = 0
	//}
	user.InnerUser.SetScore(table.Table.GetGameNum(), coinChange, table.RoomRate)
...
//跑马灯规则排序
func (table *TableLogic) orderMarqueeRules(rules []*message.MarqueeConfig) []*message.MarqueeConfig {
	orderRules := make([]*message.MarqueeConfig, 0)
...
func (table *TableLogic) createOperationLog(user *data.User, coinChange, operationType, userScore int64, bet int32, fishId string, isAbnormal bool) {
	...
	user.Log = append(user.Log, &message.GameLog{
		UserId:  userId,
		Content: content,
	})
	...
	if user2 != nil && skillId != 3 {
		...
		for _, fishId := range req.GetFishIds() {
			...
			if hitFish == nil || hitFish.hited || hitFish.info.GetSkillId() > 0 ||
				hitFish.info.GetTypeId() == msg.Type_SPECIAL ||
...
func (table *TableLogic) skill(buffer []byte, user player.PlayerInterface) {
	...
	proto.Unmarshal(buffer, req)
	skillId := req.GetSkillId()
	if u, ok := table.Users[user.GetID()]; ok {
		if !table.checkSkill(u, skillId) {
			return
		}
	} else {
		log.Errorf("Get Player %d error %v", user.GetID(), table.Users)
		panic("exit")
	}
	...
	if GetChance(chance, 0) {
		for i := 0; i < 200; i++ {
			...
			if line != nil && name != nil && len(line) > 0 && speed != nil && len(speed) > 0 { //config.GetFishType(fishId) != msg.Type_SPECIAL &&
				...
				ttt, _ := strconv.Atoi(t.(json.Number).String())
				l, s, t := table.getConfLine(line, speed, ttt)
...
//解析配置路线
func (table *TableLogic) getConfLine(lines []interface{}, speeds []interface{}, time int) ([]*msg.Point, []int32, int32) {
	line := table.getPoint(lines)
	speed := table.getArray(speeds)

	//t, _ := strconv.Atoi(time.(json.Number).String())
	return line, speed, int32(time)
}
...
func (table *TableLogic) saveScore() {
	table.RoomRate = table.Table.GetRoomRate()
	for _, v := range table.Users {
...
func (table *TableLogic) shotOff() {
	now := time.Now().UnixNano() / 1e6
	for _, u := range table.Users {
		if now-u.LastShootTime > int64(shotOffTime) && u.IsRobot { // !u.IsRobot &&
			table.shotOffGame(u)
		}
	}
}
...
func (table *TableLogic) refresh() {
	...
	//log.Debugln("refresh fish", table.FishNum, table.FishCap, table.Frozen, table.FishTide)
	for k, v := range table.FishNum {
		if v < table.FishCap[k] && !table.FishTide {
...
//机器人加入成功
func (table *TableLogic) BindRobot(ai player.RobotInterface) player.RobotHandler {
...
// 		return
// 	}
// 	robot.AI = user.BindRobotHandler(robot)
// 	//user.SendMsg(int32(msg.MsgId_ZERO), &msg.Fish{})
// }
