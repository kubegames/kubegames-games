...
func (game *DouDizhu) confirmDizhu() {
	...
	// 确认角色
	game.Dizhu.Role = data.RoleDizhu
	game.Chairs[GetNextChairID(game.Dizhu.ChairID)].Role = data.RoleDownPeasant
	game.Chairs[GetPreChairID(game.Dizhu.ChairID)].Role = data.RoleUpPeasant
...
func (game *DouDizhu) GameOver() {
	...
	for id, user := range game.UserList {
		...
		// todo 临时改动， 记得删掉
		game.UserList[id].ExitPermit = true
