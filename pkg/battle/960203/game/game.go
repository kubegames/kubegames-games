...
import (
	...
	"github.com/kubegames/kubegames-games/pkg/battle/960203/data"
...
// WatchBankerRoom 抢庄牛牛房间
type WatchBankerRoom struct {
}
...
// InitTable 初始化游戏房间
func (room *WatchBankerRoom) InitTable(table table.TableInterface) {
	//log.Tracef("init table num %d", table.GetID())
	game := new(WatchBanker)
	game.Table = table
...
// InitConfig 加载配置文件
func (game *WatchBanker) InitConfig() {
	// 加载房间配置
...
func (game *WatchBanker) OnActionUserSitDown(userInter player.PlayerInterface, orderIndex int, config string) table.MatchKind {
	...
	if _, ok := game.UserList[userID]; !ok {
		...
		if game.Status == int32(msg.GameStatus_StartMove) && game.TimerJob.GetTimeDifference() < 500 {
			log.Warnf("游戏 %d 最后0.5秒不能进入", game.Table.GetID())
			return table.SitDownErrorNomal
		}
...
//BindRobot 绑定机器人
func (game *WatchBanker) BindRobot(ai player.RobotInterface) player.RobotHandler {
	robot := new(Robot)
	if game.RobotCfg == nil {
		log.Errorf("游戏 %v 第一个玩家为机器人", game)
	} else {
		user, ok := game.UserList[ai.GetID()]
		if !ok {
			log.Warnf("获取玩家异常！！！！")
...
// SendScene 发送场景消息
func (game *WatchBanker) SendScene(userInter player.PlayerInterface) {
	userID := userInter.GetID()
...
// UserReady 用户准备
func (game *WatchBanker) UserReady(userInter player.PlayerInterface) bool {
	userID := userInter.GetID()
...
// GameStart 框架询问是否开赛
func (game *WatchBanker) GameStart() {
	if game.Status == int32(msg.GameStatus_ReadyStatus) {
		// 准备玩家人数
		readyUserCount := 0
...
// UserOffline 用户离线
func (game *WatchBanker) UserOffline(userInter player.PlayerInterface) bool {
	user, ok := game.UserList[userInter.GetID()]
	if !ok {
		log.Errorf("获取玩家异常！！！！")
		return true
...
func (game *WatchBanker) UserLeaveGame(userInter player.PlayerInterface) bool {
...
func (game *WatchBanker) OnGameMessage(subCmd int32, buffer []byte, userInter player.PlayerInterface) {
...
// ResetTable 重置桌子
func (game *WatchBanker) ResetTable() {
	// 重置桌面属性
	game.Banker = nil
	game.Poker = nil
	game.TimeCfg = nil
...
func (game *WatchBanker) CloseTable() {}
