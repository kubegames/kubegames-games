...
// UserRobBanker 用户抢庄
func (game *WatchBanker) UserRobBanker(buffer []byte, userInter player.PlayerInterface) {
	...
	if int(req.RobIndex) > len(game.GameCfg.RobOption)-1 || int(req.RobIndex) < -1 {
		log.Warnf("错误的抢注选项: %d", req.RobIndex)
		return
	}
	...
	if req.RobIndex != -1 {
		...
		// 最高抢庄倍数 = 携带金额/桌面有效玩家数（除开自己）/最高牌型5 / 底注
...
// UserRobBanker 用户投注
func (game *WatchBanker) UserBetChips(buffer []byte, userInter player.PlayerInterface) {
	// 用户ID
	userID := userInter.GetID()
	...
	if req.BetMultiple <= 0 {
		log.Warnf("错误的投注倍数: %d", req.BetMultiple)
		return
	}
...
// UserRobBanker 用户摊牌
func (game *WatchBanker) UserShowCards(buffer []byte, userInter player.PlayerInterface) {
	// 用户ID
	userID := userInter.GetID()
	...
	if allShow {
		...
		if !ok {
			log.Warnf("定时进入摊牌结束状态失败")
		}
	}
}

// UserDemandCards 要牌请求
func (game *WatchBanker) UserDemandCards(buffer []byte, userInter player.PlayerInterface) {
	// 用户ID
	userID := userInter.GetID()
