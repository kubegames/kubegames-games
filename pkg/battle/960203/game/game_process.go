...
func (game *WatchBanker) StartMoveEnd() {
	...
	if len(game.UserList) < 2 {
		...
		// 开始游戏动画后开始抢庄
		game.TimerJob, _ = game.Table.AddTimer(int64(game.TimeCfg.StartAnimation), game.StartRob)
...
// StartRob 开始抢庄
func (game *WatchBanker) StartRob() {
	...
	for id, user := range game.UserList {
		...
		if user.User.IsRobot() {
			cards = ControlSort(cards, cardsType)
		}
		...
		if len(user.HoldCards.Cards) == 0 {
			user.HoldCards = &poker.HoldCards{
				Cards:             cards,
				CardsType:         cardsType,
				FirstHalfCards:    cards[:4],
				LowerHalfCards:    cards[4:],
	...
	// 发牌动画
	game.TimerJob, _ = game.Table.AddTimer(int64(game.TimeCfg.FirstDealAnimation*len(game.UserList)), func() {
...
// StartBet 开始投注
func (game *WatchBanker) StartBet() {
...
// DealCards 发牌
func (game *WatchBanker) DealCards() {
...
// StartShow 开始摊牌
func (game *WatchBanker) StartShow() {
...
func (game *WatchBanker) Settle() {
	...
	for k, v := range resultList {
		user, ok := game.UserList[v.UserId]
		if !ok {
			log.Errorf("查询用户 %d 失败", v.UserId)
			continue
		}
...
// GameOver 游戏结束
func (game *WatchBanker) GameOver() {
