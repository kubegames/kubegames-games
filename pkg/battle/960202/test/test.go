...
type Params struct {
	...
	RobRate     int   `json:"rob_rate"`     // 做庄概率
}
...
func (game *testGame) Settle() {
	...
	for id := range game.UserList {
		...
		if game.ControlledCards[game.Banker.ID] == nil || game.ControlledCards[id] == nil {
			fmt.Print(game.Banker.ID)
			fmt.Print(id)
			fmt.Print("\n")
			fmt.Println(game.ControlledCards)
			fmt.Println(game.UserList)
		}
